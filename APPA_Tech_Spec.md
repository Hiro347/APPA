...
|---|---|---|---|
| **Semua Data Pasar & Harga (Prioritas Utama)** | **SearchArsenal Multi-Tier (Google Custom Search API $\rightarrow$ Yahoo HTML $\rightarrow$ DDG HTML $\rightarrow$ SearXNG)** + Web Scraping | Web Scraping (HTML -> **Markdown**) | **Strategi Agentic Web Search:** Agent mencari *query* melalui *SearchArsenal* yang menggabungkan Google Custom Search API, Yahoo Search (`site:` operator injection), DDG HTML scraper, dan SearXNG secara paralel non-blocking tanpa event loop delay. Hasil *snippet* & Crawl4AI deep scrape disaring secara teruji:<br/>1. **PruningContentFilter (Threshold 0.48)**: Memangkas boilerplate navbars/footers sambil mempertahankan tabel harga terstruktur.<br/>2. **Verbatim LLM Price Extraction & String Match Guardrail**: LLM mengekstrak harga persis (*verbatim*) dari teks sumber, tanpa kalkulasi matematika otomatis Python yang merusak harga nominal. Hasil ekstraksi diverifikasi via `_verify_price_in_text` untuk menjamin 0% halusinasi.<br/>3. **Context Window Protection**: Teks markdown dibatasi maksimal 12.000 karakter sebelum diproses SLM. |
